...
router.post("/products", validate(productCreateSchema), async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (images.length > 0) {
    await supabase.from("product_images").insert(
      images.map((img: any) => ({ ...img, product_id: product.id }))
    );
  }

  if (sizes.length > 0) {
    await supabase.from("product_sizes").insert(
      sizes.map((sz: any) => ({ ...sz, product_id: product.id }))
    );
  }
...
router.put("/products/:id", validate(productUpdateSchema), async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (images) {
    ...
    await supabase.from("product_images").insert(
      images.map((img: any) => ({ ...img, product_id: id }))
    );
  }
...
router.post("/categories", validate(categoryCreateSchema), async (req: AuthenticatedRequest, res: Response) => {
  const { data, error } = await supabase
    .from("categories")
    .insert(req.body)
    .select()
...
router.put("/categories/:id", validate(categoryCreateSchema), async (req: AuthenticatedRequest, res: Response) => {
  const id = req.params.id as string;
...
// Orders
router.get("/orders", async (_req: AuthenticatedRequest, res: Response) => {
  const { data, error } = await supabase
    .from("orders")
...
router.put("/orders/:id/status", validate(orderStatusUpdateSchema), async (req: AuthenticatedRequest, res: Response) => {
  const id = req.params.id as string;
...
router.post("/coupons", validate(couponCreateSchema), async (req: AuthenticatedRequest, res: Response) => {
  const { data, error } = await supabase
    .from("coupons")
    .insert(req.body)
    .select()
...
router.put("/coupons/:id", validate(couponCreateSchema), async (req: AuthenticatedRequest, res: Response) => {
  const id = req.params.id as string;
